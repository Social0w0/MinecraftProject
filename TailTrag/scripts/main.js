...
function startAirdropTimer() {
    const AIRDROP_INTERVAL = 18000; // 15분
...
function startRevealTimer() {
    // 타이머 하나로 단계 관리
    // 1단계: WARN_TICKS 후 경고 메시지
    // 2단계: 경고 후 WARN_TO_REVEAL_TICKS 뒤 실제 공개
    // 이후 1단계부터 반복
    const WARN_TICKS          = 24000; // 경고까지 대기 (테스트: 600 / 실제: 16800)
    const WARN_TO_REVEAL_TICKS = 1200; // 경고 후 공개까지 대기 (테스트: 600 / 실제: 1200)

    let phase = 0; // 0 = 경고 대기, 1 = 공개 대기
    let elapsed = 0;

    revealTimerId = system.runInterval(() => {
        if (!gameRunning) return;

        elapsed += 5; // 5틱마다 실행되므로 5씩 증가

        if (phase === 0 && elapsed >= WARN_TICKS) {
            // 경고 메시지
        ...
        else if (phase === 1 && elapsed >= WARN_TO_REVEAL_TICKS) {
            // 좌표 공개
            const targets = getPlayers();
            if (targets.length > 0) {
                const chosen = targets[Math.floor(Math.random() * targets.length)];
                const fx = Math.floor(chosen.location.x);
                const fy = Math.floor(chosen.location.y);
                const fz = Math.floor(chosen.location.z);
                world.sendMessage(`§c[좌표 공개] §e${chosen.name}§f 의 현재 좌표: §b(${fx}, ${fy}, ${fz})`);
                world.getDimension("overworld").runCommand(
    ...
    // revealWarnId는 더 이상 사용 안 하므로 -1 유지
    revealWarnId = -1;
}
...
world.beforeEvents.itemUse.subscribe(ev => {
    ...
    // beforeEvents에서는 ev.cancel만 처리하고
    // 나머지 모든 로직은 system.run()으로 넘깁니다.
    ev.cancel = true;
    ...
    system.run(() => {
        ...
        for (let i = 0; i < inv.size; i++) {
            ...
            if (slot?.typeId === "minecraft:iron_ingot") ironCount += slot.amount;
        }

        if (ironCount < 10) {
            player.onScreenDisplay.setActionBar("§c철 주괴 10개가 필요합니다!");
            try { player.runCommand("playsound random.anvil_land @s ~~~ 1 1 1"); } catch (_) {}
            return;
        }

        // ── 철괴 10개 인벤토리 API로 직접 차감 ──
        let toRemove = 10;
        for (let i = 0; i < inv.size && toRemove > 0; i++) {
            const slot = inv.getItem(i);
            if (!slot || slot.typeId !== "minecraft:iron_ingot") continue;

            if (slot.amount <= toRemove) {
                toRemove -= slot.amount;
                inv.setItem(i, undefined); // 슬롯 비우기
            } else {
        ...
        // ── 활성화 ──
        try { player.runCommand("playsound beacon.activate @s ~~~ 1 1 1"); } catch (_) {}
        player.sendMessage("§b[꼬리잡기] 나침반이 60초간 활성화됩니다!");
...
// ─── 메인 루프 (5틱마다) ─────────────────────
// ⚠ runInterval 콜백은 read-only context이므로
//   setActionBar / teleport 등 월드 변경 API는
//   반드시 system.run() 안에서 호출해야 합니다.
system.runInterval(() => {
    if (!gameRunning) return;
    ...
    // 이번 틱에 각 플레이어에게 표시할 액션바 메시지를 수집한 뒤
    // system.run() 한 번으로 일괄 적용합니다.
    /** @type {Map<string, string>} playerName → message */
    const actionBarQueue = new Map();
    ...
    for (const player of allPlayers) {
        ...
        if (oob) oobList.push({ player, nx, nz });
    }

    // ─── 팀원 거리 이탈 데미지 계산 ──────────
    /** @type {{ member, leaderName, over }[]} */
    const leashViolators = [];
    for (const [leaderName, members] of teamMap) {
        ...
        for (const memberName of members) {
            ...
            if (dist3d > LEASH_DIST) {
                leashViolators.push({ member, leaderName, over: Math.floor(dist3d - LEASH_DIST) });
            }
        }
    }
    ...
    for (const player of allPlayers) {
        ...
        const target = allPlayers.find(p => p.name === targetName);
        if (!target) {
            compassMsgs.push({ player, msg: `§7[목표: §f${targetName}§7] (오프라인)` });
            continue;
        }
        ...
        let yawConverted  = ((player.getRotation().y % 360) + 360) % 360;
        ...
        compassMsgs.push({ player, msg: `§e${targetName} §f| ${direction} | §b${dist}m §7(${remaining}초)` });
    }

    // ─── 실제 적용: system.run() 안에서 일괄 처리 ──
    system.run(() => {
        // 구역 이탈 → teleport + 액션바
        for (const { player, nx, nz } of oobList) {
            const { y } = player.location;
            try { player.teleport({ x: nx, y, z: nz }, { dimension: player.dimension }); } catch (_) {}
            actionBarQueue.set(player.name, "§c⚠ 구역 경계를 벗어났습니다!");
        }

        // 스폰포인트 갱신
        for (const player of allPlayers) {
            try { player.runCommand("spawnpoint @s ~~~"); } catch (_) {}
        }

        // 팀원 거리 이탈 → 데미지 + 액션바
        for (const { member, leaderName, over } of leashViolators) {
            try { member.runCommand("damage @s 1 void"); } catch (_) {}
            // 나침반 메시지보다 우선순위 낮음 (나침반이 덮어씀)
            if (!actionBarQueue.has(member.name)) {
                actionBarQueue.set(member.name,
        ...
        // 접근 감지 → 사운드 + 액션바
        for (const { target } of approachAlerts) {
            try { target.runCommand("playsound note.pling @s ~ ~ ~ 1 0.5"); } catch (_) {}
            actionBarQueue.set(target.name, "§c⚠ 누군가가 당신을 찾아왔습니다!");
        }

        // 나침반 방향 (최우선 — 다른 메시지 덮어씀)
        for (const { player, msg } of compassMsgs) {
            actionBarQueue.set(player.name, msg);
        }

        // 액션바 일괄 적용
        for (const [name, msg] of actionBarQueue) {
            const p = allPlayers.find(pl => pl.name === name);
            if (p) try { p.onScreenDisplay.setActionBar(msg); } catch (_) {}
        }
    });
